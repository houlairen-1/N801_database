function ensemble2_CvsS1andS2(cover_features_path,stego_features_path_1,stego_features_path_2,log_dir,repeated_times,train_samples_percent,init_seed,d_sub)
%% C-S1-S2    
    if nargin<4+1
        repeated_times=10;
    end
    ...
    names = intersect(intersect(cover.names,stego_1.names), stego_2.names);
    names = sort(names);
    ...
    fprintf('\n\n-----------------------------------------\n');
    fprintf('%s vs. %s:\n', ...
            cover_features_name,stego_features_name_1);
    ...
    for seed=init_seed:init_seed+repeated_times-1
        ...
        TRN_stego = S1(training_set,:);
        
        % Prepare testing features
        TST_cover = C(testing_set,:);
        TST_stego = S2(testing_set,:);
