function SGRM(img_path,t_file,img_count)
    ...
    if isdir(img_path)==0
        disp('(SGRM)The source image directory is not exist!');
        return;
    end    
    ...
    if len_list<1
        disp('(SGRM)No any file in the source image directory!');
        return;
    end
    ...
    parfor i = 1:img_count
        ...
        % %Run default GSGRM extraction
        disp(['(SGRM)extracting ' img_path '/' img_name '...']);
        i_0=imread(f_name);
        fea_3d=f_steer_SPAM(i_0);
        feature_s(i,:)=fea_3d;
    end
