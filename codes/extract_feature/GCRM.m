function f=GCRM(img_path,t_file,img_count)
% %By: Qin Xinghong
% %Date: 2017.03.15
...
% % of (2  25 + 10  45)  6 = 3000 features. 
    ...
    if isdir(img_path)==0
        ...
        disp('(3D_RGB)The source image directory is not exist!');
        return;
    end    
    ...
    if len_list<1
        ...
        disp('(3D_RGB)No any file in the source image directory!');
        return;
    end
...
%     for i = 1:img_count
    parfor i = 1:img_count
        ...
        disp(['(3D-RGB)extracting the ' num2str(i) ', image: ' img_name '...']);
        i_0=imread(f_name);
        fea_3d=rgb_3d(i_0);
        feature_s(i,:)=fea_3d;
    end
...
% %q=[0:1; 0:3; 0:5; 0:7; 0:9; 1]    
    ...
    q=[0.1 0.3 0.5 0.7 0.9 1];
    f=zeros(1,6000);
    D_0=X(1:s1-1,1:s2-1,:)-X(1:s1-1,2:s2,:);%[1 -1]
    D_1=X(1:s1-1,1:s2-1,:)-X(2:s1,1:s2-1,:);%v:[1 -1]
    D_n=sqrt(D_0.*D_0+D_1.*D_1);
    D_12=D_n(:,:,1).*D_n(:,:,2);
    D_13=D_n(:,:,1).*D_n(:,:,3);
    %C_RG
    C_RG=D_0(:,:,1).*D_0(:,:,2)./D_12;
    C_RG(D_12==0)=0;
    %C_RB
    C_RB=D_0(:,:,1).*D_0(:,:,3)./D_13;
    C_RB(D_13==0)=0;
...
%     f = post_processing(all2nd(X,q*2),'f2',q,f);    % 2nd order
%     f = post_processing(all3rd(X,q*3),'f3',q,f);    % 3rd order
%     f = post_processing(all3x3(X,q*4),'f3x3',q,f);  % 3x3
%     f = post_processing(all5x5(X,q*12),'f5x5',q,f); % 5x5
...
% This function calls Residual.m, Cooc.m, and Quant.m
    ...
    [T,order] = deal(1,4);% %[T,order] = deal(2,4);
...
function g = all3rd(X,q)
    ...
    [M N] = size(X); [I,J,T,order] = deal(3:M-2,3:N-2,1,4);% %[I,J,T,order] = deal(3:M-2,3:N-2,2,4);
